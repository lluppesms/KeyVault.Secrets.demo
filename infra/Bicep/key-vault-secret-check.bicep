...
resource checkSecretValue 'Microsoft.Resources/deploymentScripts@2020-10-01' = if (checkForDuplicateKey) {
  ...
  properties: {
    ...
    // I thought maybe if I used a URI path to a script file instead of an inline script it would run faster
    //   but it was not... so i might as well use the inline script as it's easier to read/maintain
    // Note: primaryScriptUri must be an https url, not a local file path
